...
function FormularioProducto() {
  ...
  const [producto, setProducto] = useState({producto: '', precio: '', descripcion: '', categoria: '', avatar: ''});
  const [errores, setErrores] = useState({});
  const [cargando, setCargando] = useState(false);
  const { agregarProducto,  validar } = useProducts();
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      ...
      setProducto({producto: '', precio: '', descripcion: '', categoria: '', avatar: ''});
      setErrores({});
    } catch (errores) {
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        <input
          ...
          style={{
            width: '100%',
            padding: '8px',
            border: `1px solid ${errores.producto ? 'red' : '#ccc'}`,
            borderRadius: '4px'
          }}
          placeholder="Ingrese el nombre del producto"
        />
        {errores.producto && <p style={{ color: 'red', margin: '5px 0', fontSize: '14px' }}>{errores.producto}</p>}
      </div>
